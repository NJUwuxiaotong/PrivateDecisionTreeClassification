...
class RandomDecisionTrees(DataInitialization):
    ...
    def predict_class_value_of_record_by_random(self, record):
        class_value_statistics = dict()
        for i in range(self.rdt_num):
            ...
            total_sum = sum(class_value.values())
            for key_class, value in class_value.items():
                if key_class in class_label_probabilities.keys():
                    class_label_probabilities[key_class] += value / total_sum
                else:
                    class_label_probabilities[key_class] = value / total_sum

            class_keys = list(class_label_probabilities.keys())
            class_probabilities = list(class_label_probabilities.values())
            chosen_class = pf.generate_random_from_specified_pr(
                class_probabilities)
            chosen_class = class_keys[chosen_class]
            if chosen_class in class_value_statistics.keys():
                class_value_statistics[chosen_class] += 1
            else:
                class_value_statistics[chosen_class] = 1
    ...
    def predict_class_value_of_record_by_probabilities(self, record):
        predicted_probabilities = self.check_class_value_of_record(record)
        class_keys = list(predicted_probabilities.keys())
        probabilities = list(predicted_probabilities.values())
        chosen_class = pf.generate_random_from_specified_pr(probabilities)
        chosen_class = class_keys[chosen_class]
        return chosen_class
    ...
    def predict_class_value_of_record(self, record):
        ...
        # return self.predict_class_value_of_record_by_random(record)
        # return self.predict_class_value_of_record_by_probabilities(record)
